...
export default (urlParams) => {
	...
	// [bk-ui patch] Las rutas de assets eran relativas a la página. Vendorizado
	// bajo una subruta, se resuelven contra la ubicación real del motor. Ver PATCHES.md.
	const ENGINE_BASE = new URL("../", import.meta.url).href;
	for (const key of ["glyphMSDFURL", "glintMSDFURL", "baseTextureURL", "glintTextureURL"]) {
		if (typeof config[key] === "string") {
			config[key] = new URL(config[key], ENGINE_BASE).href;
		}
	}
